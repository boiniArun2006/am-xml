...
public final class Http2 {
    ...
    public final String frameLog(boolean inbound, int streamId, int length, int type, int flags) {
        String str;
        String str2 = formattedType$okhttp(type);
        String flags2 = formatFlags(type, flags);
        if (inbound) {
            str = "<<";
        } else {
            str = ">>";
        }
        return Util.format("%s 0x%08x %5d %-13s %s", str, Integer.valueOf(streamId), Integer.valueOf(length), str2, flags2);
    }
}
