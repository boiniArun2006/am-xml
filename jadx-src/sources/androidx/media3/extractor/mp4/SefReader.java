...
final class SefReader {
    ...
    private static final l3D COLON_SPLITTER = l3D.nr(':');
    private static final l3D ASTERISK_SPLITTER = l3D.nr('*');
    ...
    private static SlowMotionData readSlowMotionData(ParsableByteArray parsableByteArray, int i2) throws ParserException {
        ...
        List listJ2 = ASTERISK_SPLITTER.J2(parsableByteArray.readString(i2));
        for (int i3 = 0; i3 < listJ2.size(); i3++) {
            List listJ22 = COLON_SPLITTER.J2((CharSequence) listJ2.get(i3));
            if (listJ22.size() != 3) {
    ...
    private void readSefData(ExtractorInput extractorInput, List<Metadata.Entry> list) throws IOException {
        ...
        for (int i2 = 0; i2 < this.dataReferences.size(); i2++) {
            ...
            if (iNameToDataType != TYPE_SLOW_MOTION_DATA) {
                if (iNameToDataType != TYPE_SUPER_SLOW_MOTION_DATA && iNameToDataType != TYPE_SUPER_SLOW_MOTION_BGM && iNameToDataType != TYPE_SUPER_SLOW_MOTION_EDIT_DATA && iNameToDataType != TYPE_SUPER_SLOW_DEFLICKERING_ON) {
                    throw new IllegalStateException();
                }
            } else {
