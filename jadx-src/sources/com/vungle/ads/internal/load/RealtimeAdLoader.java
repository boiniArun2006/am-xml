...
public final class RealtimeAdLoader extends BaseAdLoader {
    ...
    private final void sendWinNotification(List<String> notifications) {
        if (notifications == null || !notifications.isEmpty()) {
            ...
            if (notifications != null) {
                ...
                while (it.hasNext()) {
                    TpatSender.sendTpat$default(m248sendWinNotification$lambda2(lazy), new TpatRequest.Builder((String) it.next()).tpatKey(Constants.WIN_NOTIFICATION).withLogEntry(getLogEntry()).build(), false, 2, null);
                }
            }
        }
    }
    ...
    private static final VungleApiClient m247requestAd$lambda0(Lazy<VungleApiClient> lazy) {
        return lazy.getValue();
    }

    /* JADX INFO: renamed from: sendWinNotification$lambda-2, reason: not valid java name */
    private static final TpatSender m248sendWinNotification$lambda2(Lazy<TpatSender> lazy) {
        return lazy.getValue();
    }

    @Override // com.vungle.ads.internal.load.BaseAdLoader
    public void onAdLoadReady() {
        List<String> winNotifications;
        AdPayload advertisement = getAdvertisement();
        if (advertisement != null) {
            winNotifications = advertisement.getWinNotifications();
        } else {
            winNotifications = null;
        }
        sendWinNotification(winNotifications);
    }
        ...
        if (ConfigManager.INSTANCE.rtaDebuggingEnabled()) {
            try {
                ...
                if (decodedAdsResponse != null) {
                    new RTADebugger(m247requestAd$lambda0(lazy)).reportAdMarkup(decodedAdsResponse);
                }
