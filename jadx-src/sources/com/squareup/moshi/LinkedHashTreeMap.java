...
final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
    ...
    Node<K, V> find(K k2, boolean z2) {
        ...
        if (node2 != null) {
            ...
            node = new Node<>(node5, k2, iSecondaryHash, node4, node4.prev);
            if (i2 < 0) {
                node5.left = node;
            } else {
                node5.right = node;
            ...
            node = new Node<>(node2, k2, iSecondaryHash, node4, node4.prev);
            nodeArr[length] = node;
        }
        int i3 = this.size;
        this.size = i3 + 1;
    ...
    static <K, V> Node<K, V>[] doubleCapacity(Node<K, V>[] nodeArr) {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            ...
            if (node != null) {
                ...
                nodeArr2[i2 + length] = i5 > 0 ? avlBuilder2.root() : null;
            }
        }
        return nodeArr2;
    }
