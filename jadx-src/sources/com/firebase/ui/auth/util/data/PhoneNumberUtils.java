...
public abstract class PhoneNumberUtils {
    ...
    public static PhoneNumber getPhoneNumber(@NonNull String str) {
        String countryCodeForPhoneNumberOrDefault = DEFAULT_COUNTRY_CODE;
        String country = DEFAULT_LOCALE.getCountry();
        if (str.startsWith("+")) {
            countryCodeForPhoneNumberOrDefault = getCountryCodeForPhoneNumberOrDefault(str);
            country = getCountryIsoForCountryCode(countryCodeForPhoneNumberOrDefault);
            str = stripCountryCode(str, countryCodeForPhoneNumberOrDefault);
        }
        return new PhoneNumber(str, country, countryCodeForPhoneNumberOrDefault);
    }

    private static SparseArray<List<String>> createCountryCodeToRegionCodeMap() {
        SparseArray<List<String>> sparseArray = new SparseArray<>(Sdk.SDKError.Reason.AD_RESPONSE_EMPTY_VALUE);
        ...
        sparseArray.put(46, Collections.singletonList(FlTC.Dseylz));
        sparseArray.put(47, Arrays.asList("NO", "SJ"));
        sparseArray.put(48, Collections.singletonList("PL"));
        sparseArray.put(49, Collections.singletonList(qUrazMnwDskFs.byw));
        ...
        sparseArray.put(91, Collections.singletonList(HlsMediaPlaylist.Interstitial.SNAP_TYPE_IN));
        ...
        sparseArray.put(Sdk.SDKError.Reason.PLACEMENT_SLEEP_VALUE, Arrays.asList(RequestConfiguration.MAX_AD_CONTENT_RATING_MA, "EH"));
        sparseArray.put(Sdk.SDKError.Reason.INVALID_ADUNIT_BID_PAYLOAD_VALUE, Collections.singletonList("DZ"));
        sparseArray.put(Sdk.SDKError.Reason.AD_RESPONSE_INVALID_TEMPLATE_TYPE_VALUE, Collections.singletonList("TN"));
        sparseArray.put(Sdk.SDKError.Reason.MRAID_JS_DOES_NOT_EXIST_VALUE, Collections.singletonList("LY"));
        sparseArray.put(Sdk.SDKError.Reason.AD_RESPONSE_RETRY_AFTER_VALUE, Collections.singletonList("GM"));
        sparseArray.put(Sdk.SDKError.Reason.AD_LOAD_FAIL_RETRY_AFTER_VALUE, Collections.singletonList("SN"));
        sparseArray.put(Sdk.SDKError.Reason.INVALID_WATERFALL_PLACEMENT_ID_VALUE, Collections.singletonList("MR"));
        sparseArray.put(Sdk.SDKError.Reason.STALE_CACHED_RESPONSE_VALUE, Collections.singletonList("ML"));
        sparseArray.put(224, Collections.singletonList("GN"));
        sparseArray.put(Sdk.SDKError.Reason.AD_LOAD_FAIL_PLACEMENT_ID_MISMATCH_VALUE, Collections.singletonList("CI"));
        sparseArray.put(Sdk.SDKError.Reason.PRIVACY_ICON_FALLBACK_ERROR_VALUE, Collections.singletonList("BF"));
        ...
        sparseArray.put(376, Collections.singletonList(IYJfqUyym.cbZgEU));
    ...
    private static String getCountryCodeForPhoneNumber(String str) {
        ...
        for (int i2 = 1; i2 <= 3 && i2 <= length; i2++) {
            String strSubstring = strReplaceFirst.substring(0, i2);
            if (COUNTRY_TO_REGION_CODES.indexOfKey(Integer.valueOf(strSubstring).intValue()) >= 0) {
                return strSubstring;
            }
        }
        return null;
    ...
    private static void initCountryCodeByIsoMap() {
        ...
        while (true) {
            ...
            for (String str : sparseArray.get(iKeyAt)) {
                if (!str.equals("001")) {
                    if (map.containsKey(str)) {
                        throw new IllegalStateException("Duplicate regions for country code: " + iKeyAt);
                    }
    ...
    private static String getCountryCodeForPhoneNumberOrDefault(String str) {
        ...
        return countryCodeForPhoneNumber;
    }
