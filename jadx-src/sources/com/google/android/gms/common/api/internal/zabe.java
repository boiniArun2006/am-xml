...
public final class zabe extends GoogleApiClient implements zabz {
    ...
    public final <A extends Api.AnyClient, T extends BaseImplementation.ApiMethodImpl<? extends Result, A>> T execute(@NonNull T t3) {
        Map map = this.zac;
        Api<?> api = t3.getApi();
        Preconditions.checkArgument(map.containsKey(t3.getClientKey()), "GoogleApiClient is not configured to use " + (api != null ? api.zad() : "the API") + " required for this call.");
        this.zaj.lock();
        try {
            zaca zacaVar = this.zal;
            if (zacaVar == null) {
    ...
    public final <A extends Api.AnyClient, R extends Result, T extends BaseImplementation.ApiMethodImpl<R, A>> T enqueue(@NonNull T t3) {
        String strZad;
        Api<?> api = t3.getApi();
        boolean zContainsKey = this.zac.containsKey(t3.getClientKey());
        if (api != null) {
            strZad = api.zad();
        } else {
            strZad = "the API";
        }
        Preconditions.checkArgument(zContainsKey, "GoogleApiClient is not configured to use " + strZad + " required for this call.");
        this.zaj.lock();
        try {
            zaca zacaVar = this.zal;
            if (zacaVar == null) {
