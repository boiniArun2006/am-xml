...
public final class zzcgc extends Thread implements SurfaceTexture.OnFrameAvailableListener, zzcga {
    ...
    private static final void zzi(float[] fArr, float[] fArr2, float[] fArr3) {
        ...
        float f27 = fArr2[6];
        fArr[7] = (f21 * f27) + (f24 * fArr3[4]) + (f11 * f26);
        fArr[8] = (f27 * f22) + (fArr2[7] * fArr3[5]) + (f26 * f15);
    }

    private static final void zzj(float[] fArr, float f3) {
        fArr[0] = 1.0f;
        fArr[1] = 0.0f;
        fArr[2] = 0.0f;
        fArr[3] = 0.0f;
    ...
    private static final void zzk(float[] fArr, float f3) {
        double d2 = f3;
    ...
    public final void zzf(float f3, float f4) {
        ...
        float f5 = i2;
        this.zzk -= (f3 * 1.7453293f) / f5;
        float f6 = this.zzl - ((f4 * 1.7453293f) / f5);
        this.zzl = f6;
        if (f6 < -1.5707964f) {
            this.zzl = -1.5707964f;
            f6 = -1.5707964f;
    ...
    */
    public final void run() {
        ...
        int iZzl = zzl(35633, !((String) com.google.android.gms.ads.internal.client.zzbd.zzc().zzd(zzbgvVar)).equals(zzbgvVar.zzf()) ? (String) com.google.android.gms.ads.internal.client.zzbd.zzc().zzd(zzbgvVar) : "attribute highp vec3 aPosition;varying vec3 pos;void main() {  gl_Position = vec4(aPosition, 1.0);  pos = aPosition;}");
        if (iZzl == 0) {
            iGlCreateProgram = 0;
        } else {
            zzbgv zzbgvVar2 = zzbhe.zzbR;
            int iZzl2 = zzl(35632, !((String) com.google.android.gms.ads.internal.client.zzbd.zzc().zzd(zzbgvVar2)).equals(zzbgvVar2.zzf()) ? (String) com.google.android.gms.ads.internal.client.zzbd.zzc().zzd(zzbgvVar2) : "#extension GL_OES_EGL_image_external : require\n#define INV_PI 0.3183\nprecision highp float;varying vec3 pos;uniform samplerExternalOES uSplr;uniform mat3 uVMat;uniform float uFOVx;uniform float uFOVy;void main() {  vec3 ray = vec3(pos.x * tan(uFOVx), pos.y * tan(uFOVy), -1);  ray = (uVMat * ray).xyz;  ray = normalize(ray);  vec2 texCrd = vec2(    0.5 + atan(ray.x, - ray.z) * INV_PI * 0.5, acos(ray.y) * INV_PI);  gl_FragColor = vec4(texture2D(uSplr, texCrd).xyz, 1.0);}");
            if (iZzl2 != 0) {
        ...
        zzh("bindTextures");
        GLES20.glTexParameteri(36197, 10240, androidx.media3.common.C.TEXTURE_MIN_FILTER_LINEAR);
        zzh("texParameteri");
        GLES20.glTexParameteri(36197, 10241, androidx.media3.common.C.TEXTURE_MIN_FILTER_LINEAR);
        zzh("texParameteri");
        GLES20.glTexParameteri(36197, 10242, 33071);
        zzh("texParameteri");
        GLES20.glTexParameteri(36197, 10243, 33071);
        zzh("texParameteri");
        int iGlGetUniformLocation = GLES20.glGetUniformLocation(this.zzq, jhotmBbwMbr.amLTROom);
                ...
                while (!this.zzB) {
                    ...
                    if (zzcgbVar.zze(fArr)) {
                        if (Float.isNaN(this.zzj)) {
                            ...
                            float f5 = fArr[1];
                            float f6 = fArr2[1];
                            float f7 = (f3 * f4) + (f5 * f6) + (fArr[c2] * 0.0f);
                            float f8 = (fArr[i2] * f4) + (fArr[4] * f6) + (fArr[5] * 0.0f);
                            float f9 = (fArr[6] * f4) + (fArr[7] * f6) + (fArr[8] * 0.0f);
