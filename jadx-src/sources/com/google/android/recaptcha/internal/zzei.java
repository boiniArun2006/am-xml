...
public final class zzei extends SQLiteOpenHelper {
    ...
    public final List zzd() {
        ...
        while (cursorQuery.moveToNext()) {
            try {
                try {
                    int i2 = cursorQuery.getInt(cursorQuery.getColumnIndexOrThrow("id"));
                    String string = cursorQuery.getString(cursorQuery.getColumnIndexOrThrow("ss"));
                    long j2 = cursorQuery.getLong(cursorQuery.getColumnIndexOrThrow(a.f62812b));
                    Intrinsics.checkNotNull(string);
                    arrayList.add(new zzej(string, j2, i2));
                } catch (Exception unused) {
    ...
    public final boolean zzf(zzej zzejVar) {
        if (zza(CollectionsKt.listOf(zzejVar)) == 1) {
            return true;
        }
        return false;
    }
