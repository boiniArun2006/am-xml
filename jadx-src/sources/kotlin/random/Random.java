...
public abstract class Random {
    ...
    public byte[] nextBytes(byte[] array, int fromIndex, int toIndex) {
        ...
        for (int i7 = 0; i7 < i5; i7++) {
            array[fromIndex + i7] = (byte) (iNextBits >>> (i7 * 8));
        }
        return array;
    }
    ...
    public static /* synthetic */ byte[] nextBytes$default(Random random, byte[] bArr, int i2, int i3, int i5, Object obj) {
        if (obj != null) {
    ...
    public double nextDouble(double from, double until) {
        ...
        if (Double.isInfinite(d2) && !Double.isInfinite(from) && !Double.isNaN(from) && !Double.isInfinite(until) && !Double.isNaN(until)) {
            double d4 = 2;
            double dNextDouble2 = nextDouble() * ((until / d4) - (from / d4));
