    ...
    public int compare(DeclarationDescriptor declarationDescriptor, DeclarationDescriptor declarationDescriptor2) {
        ...
        if ((declarationDescriptor instanceof TypeAliasDescriptor) && (declarationDescriptor2 instanceof TypeAliasDescriptor)) {
            ...
            for (int i2 = 0; i2 < Math.min(valueParameters.size(), valueParameters2.size()); i2++) {
                ...
                int iCompareTo3 = descriptorRenderer3.renderType(valueParameters.get(i2).getType()).compareTo(descriptorRenderer3.renderType(valueParameters2.get(i2).getType()));
                if (iCompareTo3 != 0) {
                    return iCompareTo3;
                }
            }
            ...
            for (int i3 = 0; i3 < Math.min(typeParameters.size(), typeParameters2.size()); i3++) {
                List<KotlinType> upperBounds = typeParameters.get(i3).getUpperBounds();
                List<KotlinType> upperBounds2 = typeParameters2.get(i3).getUpperBounds();
                ...
                for (int i5 = 0; i5 < upperBounds.size(); i5++) {
                    ...
                    int iCompareTo4 = descriptorRenderer4.renderType(upperBounds.get(i5)).compareTo(descriptorRenderer4.renderType(upperBounds2.get(i5)));
                    if (iCompareTo4 != 0) {
                        return iCompareTo4;
                    }
                }
