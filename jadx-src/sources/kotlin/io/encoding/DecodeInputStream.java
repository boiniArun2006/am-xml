...
final class DecodeInputStream extends InputStream {
    ...
    public int read() throws IOException {
        ...
        int i5 = read(this.singleByteBuffer, 0, 1);
        if (i5 == -1) {
            return -1;
        }
        if (i5 == 1) {
    ...
    */
    public int read(byte[] destination, int offset, int length) throws IOException {
        ...
        if (offset >= 0 && length >= 0 && (i2 = offset + length) <= destination.length) {
            if (!this.isClosed) {
                if (this.isEOF) {
                    return -1;
                }
                ...
                while (true) {
                    ...
                    iDecodeSymbolBufferInto += decodeSymbolBufferInto(destination, iDecodeSymbolBufferInto, i2, iHandlePaddingSymbol);
                }
            } else {
